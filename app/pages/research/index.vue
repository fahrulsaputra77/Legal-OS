...
const processResearchQuery = (query: string) => {
  ...
  if (q.includes('phk') || q.includes('pesangon') || q.includes('kerja')) {
    ...
    confidence = 94
    synthesisHtml = `<p class="mb-3">Berdasarkan analisis terhadap <strong>${matchedDocs.length} regulasi dan yurisprudensi</strong>, hak pesangon untuk karyawan yang di-PHK secara sepihak diatur secara tegas dalam <strong>UU Ketenagakerjaan</strong> dan diperbarui melalui <strong>PP 35/2021</strong>.</p>
    ...
    confidence = 89
    synthesisHtml = `<p class="mb-3">Kasus yang melibatkan unsur penipuan atau penggelapan diatur dalam <strong>KUHP</strong>.</p>
    <ul class="space-y-2 mb-3 list-disc pl-5">
    ...
    confidence = 92
    synthesisHtml = `<p class="mb-3">Gugatan atas Perbuatan Melawan Hukum (PMH) dapat diajukan berdasarkan <strong>Pasal 1365 KUHPerdata</strong>.</p>
    <ul class="space-y-2 mb-3 list-disc pl-5">
    ...
    confidence = 85
    synthesisHtml = `<p class="mb-3">Pencemaran nama baik di media elektronik diatur secara khusus dalam <strong>UU ITE</strong>.</p>
    <ul class="space-y-2 mb-3 list-disc pl-5">
    ...
    confidence = 72
    synthesisHtml = `<p class="mb-3">Berdasarkan kata kunci <strong>"${query}"</strong>, sistem menemukan beberapa dokumen hukum yang mungkin relevan secara kontekstual.</p>
    <ul class="space-y-2 mb-3 list-disc pl-5">
...
const confidenceScore = ref<number>(0)
const hasSearched = ref(false)
const selectedDocId = ref<string | null>(null)
const activeSourceTypes = ref<string[]>([])
const activeYears = ref<string[]>([])

const selectedDoc = computed(() => resultDocs.value.find(d => d.id === selectedDocId.value))
...
const handleSearch = (e: Event) => {
  ...
  setTimeout(() => {
    const data = processResearchQuery(searchQuery.value)
    resultDocs.value = data.docs
    synthesis.value = data.synthesisHtml
    confidenceScore.value = data.confidence
    hasSearched.value = true
    if (data.docs.length > 0) {
      selectedDocId.value = data.docs[0].id
    }
...
const toggleSourceType = (id: string) => {
  if (activeSourceTypes.value.includes(id)) {
    activeSourceTypes.value = activeSourceTypes.value.filter(t => t !== id)
  } else {
    activeSourceTypes.value.push(id)
  }
}

const toggleYear = (year: string) => {
  if (activeYears.value.includes(year)) {
    activeYears.value = activeYears.value.filter(y => y !== year)
  } else {
...
const filteredDocs = computed(() => {
  return resultDocs.value.filter(doc => {
    if (activeSourceTypes.value.length > 0 && !activeSourceTypes.value.includes(doc.sourceType)) return false
    return true
  })
})
</script>
...
<template>
  <div class="flex flex-col h-[calc(100vh-theme(spacing.20))] overflow-hidden gap-4 fade-in p-4 md:p-6 pb-20 md:pb-6">
    <!-- Header Area -->
    ...
    <div class="flex flex-1 overflow-hidden gap-6 mt-2">
      <!-- LEFT PANEL: FILTERS (Hidden on small screens) -->
      <div class="hidden lg:flex w-64 shrink-0 flex-col gap-6 overflow-y-auto pr-2 pb-8">
        <div>
          <h3 class="font-semibold text-sm text-slate-900 dark:text-white flex items-center gap-2 mb-3">
            <Filter class="h-4 w-4 text-[#C9A84C]" />
            Filter Sumber
          </h3>
          <div class="flex flex-col gap-1.5">
            <button
              v-for="source in LEGAL_SOURCE_TYPES.slice(0, 10)"
              :key="source.id"
              ...
              :class="cn(
                ...
                activeSourceTypes.includes(source.id)
                  ? 'bg-[#C9A84C]/10 text-[#C9A84C] font-medium' 
                  : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
              )"
            >
              <span class="truncate">{{ source.label }}</span>
              <div v-if="activeSourceTypes.includes(source.id)" class="h-2 w-2 rounded-full bg-[#C9A84C]" />
            </button>
          </div>
        </div>
        ...
        <div>
          <h3 class="font-semibold text-sm text-slate-900 dark:text-white flex items-center gap-2 mb-3">
            <Clock class="h-4 w-4 text-[#C9A84C]" />
            Tahun Penetapan
          </h3>
          <div class="flex flex-wrap gap-2">
            <Badge 
              v-for="year in ['2024', '2023', '2022', '2021', '2020', '< 2020']"
              :key="year"
              :variant="activeYears.includes(year) ? 'default' : 'outline'"
              :class="cn(
                'cursor-pointer',
                activeYears.includes(year) ? 'bg-[#1B2A4A] text-white hover:bg-[#1B2A4A]/90' : 'hover:bg-slate-100 dark:hover:bg-slate-800'
              )"
      ...
      <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        <form @submit.prevent="handleSearch" class="shrink-0 mb-4">
          <div class="relative">
            ...
            <Input 
              ...
              class="pl-10 pr-24 py-6 text-base bg-white dark:bg-slate-950 shadow-sm border-slate-200 dark:border-slate-800 rounded-xl focus-visible:ring-[#C9A84C]"
            />
            <Button 
              type="submit" 
              :disabled="isSearching || !searchQuery.trim()"
              class="absolute right-1.5 top-1.5 bottom-1.5 bg-[#C9A84C] hover:bg-[#A68A3D] text-white rounded-lg px-6 disabled:opacity-50"
            >
              <Loader2 v-if="isSearching" class="h-4 w-4 animate-spin" />
              <span v-else>Riset</span>
            </Button>
          </div>
        </form>

        <div v-if="isSearching" class="flex-1 flex flex-col items-center justify-center text-center p-8 text-slate-500 animate-pulse">
          <Loader2 class="h-10 w-10 text-[#C9A84C] animate-spin mb-4" />
          <h3 class="text-lg font-medium text-slate-900 dark:text-slate-200 mb-2">Memproses Kueri Riset...</h3>
          <p class="max-w-md text-sm">
            Mencari preseden, menganalisis undang-undang, dan menyusun sintesis hukum.
          </p>
        </div>
        
        <ScrollArea v-else-if="hasSearched" class="flex-1 pr-4">
          <div class="flex flex-col gap-4 pb-8">
            <!-- AI SYNTHESIS CARD -->
            <Card class="border-[#C9A84C]/30 shadow-md bg-gradient-to-br from-white to-[#C9A84C]/5 dark:from-slate-900 dark:to-[#C9A84C]/10 overflow-hidden relative fade-in">
              <div class="absolute top-0 left-0 w-1 h-full bg-[#C9A84C]" />
              <CardHeader class="pb-3">
                <div class="flex items-center justify-between">
                  <CardTitle class="flex items-center gap-2 text-[#1B2A4A] dark:text-slate-200">
                    <Sparkles class="h-5 w-5 text-[#C9A84C]" />
                    Sintesis AI
                  </CardTitle>
              ...
              <CardContent class="text-sm leading-relaxed text-slate-700 dark:text-slate-300">
                <div v-html="synthesis" />
              </CardContent>
              <CardFooter class="pt-0 flex items-center justify-between">
                ...
                <div class="flex gap-2">
                  ...
                  <Button variant="ghost" size="icon" class="h-8 w-8 text-slate-400 hover:text-green-600">
                    <ThumbsUp class="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="icon" class="h-8 w-8 text-slate-400 hover:text-red-600">
            ...
            <div class="flex items-center justify-between mt-2">
              <h3 class="text-lg font-semibold text-slate-900 dark:text-white">Dokumen Referensi Teratas</h3>
              <span class="text-sm text-slate-500">{{ filteredDocs.length }} Hasil ditemukan</span>
            </div>
            ...
            <div class="flex flex-col gap-3">
              <Card 
                ...
                :class="cn(
                  ...
                  selectedDocId === doc.id ? 'border-[#C9A84C] ring-1 ring-[#C9A84C] bg-white dark:bg-slate-900' : 'bg-white/50 dark:bg-slate-950/50'
                )"
                ...
                <CardContent class="p-4">
                  <div class="flex items-start justify-between gap-4 mb-2">
                    <div class="flex flex-wrap items-center gap-2 mb-1">
                      <Badge v-if="LEGAL_SOURCE_TYPES.find(t => t.id === doc.sourceType)" variant="secondary" :class="cn('font-medium', LEGAL_SOURCE_TYPES.find(t => t.id === doc.sourceType)?.color)">
                        {{ LEGAL_SOURCE_TYPES.find(t => t.id === doc.sourceType)?.label }}
                      </Badge>
                      <Badge variant="outline" class="text-slate-500">
                        {{ doc.year }}
                      </Badge>
                      <span class="text-xs font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-900/30 px-2 py-0.5 rounded-full flex items-center gap-1">
                        <Target class="h-3 w-3" />
                        {{ doc.relevanceScore }}% Cocok
                      </span>
                    </div>
                    <Button variant="ghost" size="icon" class="h-8 w-8 text-slate-400 group-hover:text-[#C9A84C] shrink-0">
                      <BookmarkPlus class="h-4 w-4" />
                    </Button>
                  </div>
                  
                  <h4 class="font-semibold text-[#1B2A4A] dark:text-slate-200 line-clamp-1 mb-1">
                    {{ doc.title }}
                  </h4>
                  <p class="text-sm text-slate-500 dark:text-slate-400 line-clamp-2">
                    <strong class="text-slate-700 dark:text-slate-300">{{ doc.reference }}:</strong> {{ doc.excerpt }}
                  </p>
                </CardContent>
              </Card>
            </div>
        ...
        <div v-else class="flex-1 flex flex-col items-center justify-center text-center p-8 text-slate-500">
          <div class="h-16 w-16 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mb-4">
            <BookOpen class="h-8 w-8 text-slate-400" />
          </div>
          <h3 class="text-lg font-medium text-slate-900 dark:text-slate-200 mb-2">Mulai Riset Hukum Anda</h3>
          <p class="max-w-md text-sm">
      ...
      <div v-if="hasSearched && selectedDoc" class="hidden xl:flex w-[400px] shrink-0 flex-col bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm fade-in">
        <div class="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex items-center justify-between shrink-0">
          <h3 class="font-semibold text-sm flex items-center gap-2">
            <FileText class="h-4 w-4 text-[#C9A84C]" />
            Penampil Dokumen
          </h3>
          <div class="flex gap-1">
            <Button variant="ghost" size="icon" class="h-7 w-7" title="Bagikan Referensi">
              <Share2 class="h-3.5 w-3.5 text-slate-500" />
            </Button>
            <Button variant="ghost" size="icon" class="h-7 w-7" title="Unduh PDF">
        ...
        <ScrollArea class="flex-1 p-5">
          <div class="flex flex-col gap-4">
            <div>
              <Badge variant="outline" class="mb-2 text-[#1B2A4A] border-[#1B2A4A] dark:text-slate-300 dark:border-slate-700">
            ...
            <div>
              <h4 class="font-semibold text-[#C9A84C] mb-2">{{ selectedDoc.reference }}</h4>
              <div class="prose prose-sm dark:prose-invert max-w-none prose-p:leading-relaxed prose-p:mb-4 text-slate-700 dark:text-slate-300 whitespace-pre-wrap">
                {{ selectedDoc.content }}
              </div>
            </div>

            <div class="mt-8 p-4 bg-[#1B2A4A]/5 dark:bg-white/5 rounded-lg border border-[#1B2A4A]/10 dark:border-white/10">
              ...
              <Button variant="outline" size="sm" class="w-full mt-3 flex items-center justify-center gap-2">
                Jadikan Landasan Hukum <ArrowRight class="h-3 w-3" />
              </Button>
            </div>
          </div>
        </ScrollArea>
